...
class KmlWriter:
    def __init__(self, bfn):
        ...
        self.local = "C:\\Users\\q622469\\OneDrive - BMW Group\\80_Masterarbeit\\Program\\Input\\GoogleEarthIcons\\"
    ...
    def write_style_ids(self) -> bool:
        return (
                ...
                and self.write_exit_history_sid()
                and self.write_entry_exit_sid()
                and self.write_pres_seg_sid()
                and self.write_veh_gps_path_sid()
                and self.write_veh_prop_sid()
        )
    ...
    def write_entry_exit_placemark(self, name, descr, type, gps) -> bool:
        if self.write_placemark(name, descr, type + "IconStyle", gps):
            return True
        return False

    def write_exit_history_placemark(self, name, descr, gps) -> bool:
        if self.write_placemark(name, descr, "ExitHistoryIconStyle", gps):
            return True
        return False

    def write_lids_flaps_placemark(self, name, descr, gps) -> bool:
        if self.write_placemark(name, descr, "VehiclePropertyIconStyle", gps):
            return True
        return False
    ...
    def write_pswf_sid(self) -> bool:
        return (
                self.write_icon_style("ParkenIconStyle", self.local + "icon39(3).png")
    ...
    def write_entry_exit_sid(self) -> bool:
        if (self.write_icon_style("EntryIconStyle", self.local + "icon54(2).png") and
                self.write_icon_style("ExitIconStyle", self.local + "icon7(2).png")):

            return True
        return False

    def write_exit_history_sid(self) -> bool:
        if self.write_icon_style("ExitHistoryIconStyle", self.local + "icon8(2).png"):
            return True
        return False

    def write_veh_gps_path_sid(self) -> bool:
        if self.write_line_style("VehGpsPathStyle", "ffffffff", "2"):
            return True
        return False

    def write_veh_prop_sid(self) -> bool:
        if self.write_icon_style("VehiclePropertyIconStyle", self.pal3 + "icon62.png"):
            return True
        return False
